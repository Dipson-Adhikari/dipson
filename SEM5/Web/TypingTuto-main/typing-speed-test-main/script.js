...
const starterButton = document.getElementById("starter_button");
...
{
    let difficultyContent = content[difficulty];
    console.log(difficultyContent);
    let randomIndex = Math.floor(Math.random() * difficultyContent.length);
    currentContent = difficultyContent[randomIndex];
...
function renderText()
{
    ...
    for(let i=0; i<currentContent.text.length; i++)
    {
        ...
        if(i <currentIndex)
        {
        if(currentContent.text[i] === hiddenInput.value[i])
        {
            className = "correct";
...
function updatestats()
{
    ...
    let accuracy = Math.round((correctcount/totalTyped)*100);
